...
class PGAgent(nn.Module):
    ...
    def _discounted_return(self, rewards: Sequence[float]) -> Sequence[float]:
        ...
        involve t)!
        """
        discount_list = np.array([np.power(self.gamma, i) for i in range(len(rewards))])
        new_reward_list = rewards * discount_list
        
        output = np.cumsum(new_reward_list)[-1].repeat(rewards.shape)
        return output
